    ...
    try {
      ...
      if (selectedPlan !== 'free') {
        try {
          ...
          // Continue with free plan if subscription fails
          setError('Subscription setup failed. You can upgrade later in settings.');
        }
      }
    } catch (err) {
    ...
    try {
      const { error } = await supabase.auth.signInWithOAuth({
        ...
        options: {
          ...
          queryParams: {
            selectedPlan // Pass selected plan to the OAuth provider
          }
        }
      });
      if (error) {
  ...
  return (
    <div className="min-h-screen flex">
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-gradient-to-br from-indigo-600 to-purple-700 p-12 flex-col justify-between">
        <div>
          <h1 className="text-white mb-2">JobLander</h1>
          <p className="text-indigo-100">Your career journey starts here</p>
        </div>
      ...
      {/* Right Side - Plan Selection and Signup Form */}
      <div className="flex-1 flex items-center justify-center p-8 bg-white dark:bg-gray-900">
        <div className="w-full max-w-md space-y-8">
          <div>
            <h2 className="text-gray-900 dark:text-white">Choose your plan</h2>
            <p className="mt-2 text-gray-600 dark:text-gray-400">
              Start with a free plan or unlock all features
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 gap-4 mb-8">
            {plans.map((plan) => (
              ...
              >
                {plan.popular && (
                  <span className="absolute -top-3 right-4 bg-indigo-600 text-white text-sm px-3 py-1 rounded-full">
                    Popular
                  </span>
                )}
          ...
          <div>
            <h3 className="text-gray-900 dark:text-white">Create your account</h3>
            <p className="mt-2 text-gray-600 dark:text-gray-400">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            >
              {loading 
                ...
                : selectedPlan === 'free' 
                  ? 'Create free account' 
                  : `Start free trial • $${plans.find(p => p.id === selectedPlan)?.price}/month after`}
            </Button>
          </form>
